...
class AdminController extends Controller
{
    ...
    public function stages($package = 1, $stage_id = 0)
    {
        $title = "STAGES & PACKAGES";
        $search = null;
        if (isset($_REQUEST["username"]) && !empty($_REQUEST["username"])) {
            $search = $_REQUEST["username"];
            $data["username"] = $search;
        ...
        $stage = Stage::getStageFromPackage($stage_id, $package);
        $users = User::getAllUserinStageandPackage($package, $stage->id, $search);
        $usercount =  User::getTotalUserinStageandPackage($package, $stage->id, $search);
        $data["userTitle"] = "$stage->rank Users";
        $data['package'] = Package::getPackage($package);
        ...
        $data["packages"] = Package::getAllPackages();
        $data["stages"] = Stage::getStagesFromPackage($package);
    ...
    public static function generateActivationCode(Request $request)
    {
        ...
        try {
            $package = Package::getPackage($request->package);
            $result = ActivationCode::generateActivationCodes($request->size, $request->package);
            session()->flash("success", "$result Activation Codes for $package->name Package Generated Successfully");
    ...
    public static function sendActivationCodes(Request $request)
    {
        ...
        try {
            ...
            $packageObj = Package::getPackage($package);
            $rem_codes_for_package = ActivationCode::getPackageActivationCodesCount($package, ActivationCode::$ACTIVATION_GENERATED);
            if ($rem_codes_for_package < $size) {
                session()->flash("error", "You don't have enough generated codes for $packageObj->name Package. Please generate more activation codes and try again.");
                return back()->withInput();
            }
            $result = ActivationCode::sendActivationCodes($size, $package, $userName);
            session()->flash("success", "$result Activation Codes for $packageObj->name Package Sent to $userName Successfully");
            return back()->withInput();
        } catch (Exception $e) {
            dd($e);
        }
