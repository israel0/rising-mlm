...
class Stage extends Model
{
    ...
    public static function getStageFromPackage($stage_id, $package)
    {
        return Stage::where([
            "stage_id" => $stage_id,
            "package" => $package
    ...
    public static function getStagesFromPackage($package)
    {
        return Stage::where([
            "package" => $package
        ])->get();
    ...
    public function payReferralBonus($userName, $registeredUser)
    {
        ...
        $uplineUser = User::getUser($userName);
        $uplineUserStage = Stage::getStage($uplineUser->stage);
        if ($uplineUserStage->stage_id !== 0) return;
        Transaction::createCreditTransaction(Transaction::$MATRIX_WALLET, $this->matrixBonus, $userName, "Registration of $registeredUser");
        if ($uplineUser->upline && !empty($uplineUser->upline)) {
            $indirectUpline = User::getUser($uplineUser->upline);
            if ($indirectUpline) {
                $indirectUplineStage = Stage::getStage($indirectUpline->stage);
                if ($indirectUplineStage->stage_id !== 0) return;
                Transaction::createCreditTransaction(Transaction::$MATRIX_WALLET, $this->matrixBonus, $indirectUpline->userName, "Registration of $registeredUser");
            }
        }
    ...
    // public static function getTotalUserInFeederStage($package) {
    //     return Stage::where(["package" => $package, "stage_id"=> 0])->count();
    // }

    // public static function getAllUserInFeederStage($package) {
    //     return Stage::where(["package" => $package , "stage_id"=> 0])->paginate();
    // }

    // public static function getTotalUserInStageOne($package) {
    //     return Stage::where(["package" => $package , "stage_id"=> 1])->count();
    // }

    // public static function getAllUserInStageOne($package) {
    //     return Stage::where(["package" => $package , "stage_id"=> 1])->paginate();
    // }

    // public static function getTotalUserInStageTwo($package) {
    //     return Stage::where(["package" => $package , "stage_id"=> 2])->count();
    // }

    // public static function getAllUserInStageTwo($package) {
    //     return Stage::where(["package" => $package , "stage_id"=> 2])->paginate();
    // }
