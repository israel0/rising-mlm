...
class User extends Authenticatable
{
    ...
    public static function getTotalUserinStageandPackage($package, $stage, $search = "")
    {
        $whereCondition = [];
        if (!empty($search)) {
            $whereCondition = [
    ...
    public static function getAllUserinStageandPackage($package, $stage, $search = "")
    {
        if (!empty($search)) {
            $whereCondition = [
                ["userName", "REGEXP", $search],
    ...
    public static function getAllNotifications($userName)
    {
        ...
        $activationCode = Notification::where("userName", $userName)->paginate(100);
        return $activationCode;
    }
    ...
    public static function getUplineForNewUser($sponsorName)
    {
        $userArray = [];
        array_push($userArray, $sponsorName);
    ...
    {
        return User::where("sponsorName", $username)->paginate(100);
    }

    public static function getReferralsWithLimit($username, $limit)
    {
        return User::where("sponsorName", $username)->limit($limit)->get();
    }
    ...
    {
        return User::where("sponsorName", $username)->count();
    }

    public function enterNextStage()
    {
        $packageObj = Package::getPackage($this->package);
        $currentStage = Stage::getStage($this->stage);
        $nextStage = $packageObj->getNextStage($currentStage);
        if (!$nextStage) {
            if ($this->is_package_completed) return;
            ...
            $this->is_package_completed = true;
            $this->save();
            $notification = new Notification();
            $notification->notification = "Congratulations to you. You have successfully completed all the stages in this package. Thank you for choosing Paulano Graceland";
            $notification->status = 0;
            $notification->title = "YOU HAVE COMPLETED ALL YOUR STAGES!";
